...
# PSK Modulation
# ============================================================================
psk_mod = np.cos(msg * 2 * np.pi / M) + 1j * np.sin(msg * 2 * np.pi / M)
...
psk_mod_shaped_delay = np.zeros(len(psk_mod_shaped), dtype=complex)
if delay > 0:
    psk_mod_shaped_delay[delay:] = psk_mod_shaped[:-delay]
else:
    psk_mod_shaped_delay = psk_mod_shaped.copy()

# ============================================================================
# Phase Shifting
# ============================================================================
phase_shift = np.random.rand() * 2 * np.pi - np.pi
psk_mod_shaped_delay_phase = psk_mod_shaped_delay * np.exp(1j * phase_shift)

# ============================================================================
# Adding AWGN Noise
# ============================================================================
received_signal = awgn(psk_mod_shaped_delay_phase, awgn_SNR)
...
# phase_sync, phase = mthpower_loop(time_sync, M, 4, debug_level)

# ============================================================================
# PSK Demodulation
# ============================================================================
rx_phase = np.angle(phase_sync)
demod_raw = np.round(rx_phase * M / 2 / np.pi).astype(int)
recovps_sync = np.mod(demod_raw, M)

# ============================================================================
# Error Calculation
# ============================================================================
numerrs_pskc, percentage_of_errorc = symerr(msg, recovps_sync)

if debug_level:
    plt.figure()
    plt.plot(msg - recovps_sync)
    plt.title("Error Signal")
    plt.show()

if percentage_of_errorc > 0.1:
    print("Error rate too high, restarting...")
    import sys
    sys.exit(1)
...
print(f'Percentage of Demodulation Error: {percentage_of_errorc * 100:.4f} %')
...
if debug_level:
    ...
    axes[0, 0].stem(np.arange(1, plot_d + 1), np.angle(psk_mod_os[plot_s:plot_s + plot_d]))
    ...
    axes[0, 1].stem(np.arange(1, plot_d + 1), np.angle(psk_mod_os[plot_s:plot_s + plot_d]))
    ...
    psk_mod_slice = psk_mod[plot_s // OF:plot_s // OF + plot_d // OF]
    axes[1, 0].plot(np.arange(len(time_sync_slice)), np.angle(time_sync_slice))
    axes[1, 0].stem(np.arange(len(psk_mod_slice)), np.angle(psk_mod_slice))
    ...
    axes[1, 1].stem(np.arange(len(psk_mod_slice)), np.angle(psk_mod_slice))
    ...
    axes[0, 0].plot(np.arange(plot_d), np.real(psk_mod_shaped[plot_range]))
    axes[0, 0].stem(np.arange(plot_d), np.real(psk_mod_os[plot_range]))
    axes[0, 0].legend(['real', 'real of modulated os'])
    axes[0, 0].set_title("Shaped vs Modulated")
    
    axes[0, 1].plot(np.arange(plot_d), np.imag(psk_mod_shaped[plot_range]))
    axes[0, 1].stem(np.arange(plot_d), np.imag(psk_mod_os[plot_range]))
    axes[0, 1].legend(['imag', 'imag of modulated os'])
    axes[0, 1].set_title("Shaped vs Modulated")
    
    axes[1, 0].plot(np.arange(plot_d), np.abs(psk_mod_shaped[plot_range]))
    axes[1, 0].stem(np.arange(plot_d), np.abs(psk_mod_os[plot_range]))
    axes[1, 0].legend(['abs', 'abs of modulated os'])
    axes[1, 0].set_title("Shaped vs Modulated")
    
    axes[1, 1].plot(np.arange(plot_d), np.angle(psk_mod_shaped[plot_range]))
    axes[1, 1].stem(np.arange(plot_d), np.angle(psk_mod_os[plot_range]))
    ...
    psk_mod_slice = psk_mod[plot_s // OF:plot_s // OF + plot_d // OF]
    recovps_slice = recovps_sync[plot_s // OF:plot_s // OF + plot_d // OF]
    
    axes[0, 0].stem(np.arange(len(msg_slice)), msg_slice)
    axes[0, 0].set_title("Random Generated Bits")
    
    axes[0, 1].stem(np.arange(len(psk_mod_slice)), np.angle(psk_mod_slice))
    axes[0, 1].set_title("Phase of PSK Modulated")
    
    axes[1, 0].stem(np.arange(len(recovps_slice)), recovps_slice)
    axes[1, 0].set_title("Demodulated")
    
    axes[1, 1].stem(np.arange(len(psk_mod_slice)), np.abs(psk_mod_slice))
    axes[1, 1].set_title("Abs of Modulated and Oversampled")
    
    psk_mod_os_slice = psk_mod_os[plot_s:plot_s + (plot_d // OF - 1) * OF + 1]
    axes[2, 1].stem(np.arange(len(psk_mod_os_slice)), np.angle(psk_mod_os_slice))
    ...
    channel_len = len(psk_mod_shaped[channel_range])
    
    axes[0].plot(np.arange(channel_len), np.real(psk_mod_shaped[channel_range]))
    axes[0].plot(np.arange(channel_len), np.real(psk_mod_shaped_delay[channel_range]))
    axes[0].stem(np.arange(channel_len), np.real(psk_mod_os[channel_range]))
    axes[0].legend(['Shaped', 'Delayed'])
    axes[0].set_title("Real Parts")
    
    axes[1].plot(np.arange(channel_len), np.imag(psk_mod_shaped[channel_range]))
    axes[1].plot(np.arange(channel_len), np.imag(psk_mod_shaped_delay[channel_range]))
    axes[1].stem(np.arange(channel_len), np.imag(psk_mod_os[channel_range]))
    axes[1].legend(['Shaped', 'Delayed'])
    axes[1].set_title("Imaginary Parts")
    
    axes[2].plot(np.arange(channel_len), np.abs(psk_mod_shaped_delay[channel_range]))
    axes[2].plot(np.arange(channel_len), np.abs(psk_mod_shaped_delay_phase[channel_range]))
    axes[2].plot(np.arange(channel_len), np.abs(received_signal[channel_range]))
    axes[2].stem(np.arange(channel_len), np.abs(psk_mod_os[channel_range]))
    axes[2].legend(['Delayed', 'Phase Shifted', 'Noise Added'])
    axes[2].set_title("Absolute")
    
    axes[3].plot(np.arange(channel_len), np.angle(psk_mod_shaped_delay[channel_range]))
    axes[3].plot(np.arange(channel_len), np.angle(psk_mod_shaped_delay_phase[channel_range]))
    axes[3].plot(np.arange(channel_len), np.angle(received_signal[channel_range]))
    axes[3].stem(np.arange(channel_len), np.angle(psk_mod_os[channel_range]))
    ...
    ax1.scatter(np.real(psk_mod), np.imag(psk_mod), alpha=0.5, s=1)
    ...
    ax2.scatter(np.real(psk_mod_os), np.imag(psk_mod_os), alpha=0.5, s=1)
    ...
    ax3.scatter(np.real(psk_mod_shaped), np.imag(psk_mod_shaped), alpha=0.5, s=1)
    ...
    ax4.scatter(np.real(psk_mod_shaped_delay), np.imag(psk_mod_shaped_delay), alpha=0.5, s=1)
    ...
    ax5.scatter(np.real(psk_mod_shaped_delay_phase), np.imag(psk_mod_shaped_delay_phase), alpha=0.5, s=1)
